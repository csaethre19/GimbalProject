  ...
  /* USER CODE BEGIN USART3_4_IRQn 0 */
		...
		if (ch == '\r') // Command delimiter
    {
				...
				if (strcmp(cmdBuffer, "rdpitch") == 0)
				{
					...
					USART_Transmit_Float(mpu_stationary.KalmanAnglePitch, 2); // TODO: added no idea if correct
					USART_Transmit_Newline();
				}
				else if(strcmp(cmdBuffer, "rdroll") == 0){

					USART_Transmit_String("Angle roll = ");
					USART_Transmit_Float(mpu_moving.KalmanAngleRoll, 2); // TODO: added no idea if correct					
					USART_Transmit_Newline();
				}
				else if(strcmp(cmdBuffer, "rdyaw") == 0){

					USART_Transmit_String("Angle roll = 420");
					USART_Transmit_Float(mpu_moving.KalmanAngleYaw, 2); // TODO: added no idea if correct
					USART_Transmit_Newline();
				}
					...
					if(endPtr != extractedString){
						...
						set_desiredPitch(value); // TODO: added no idea if correct
						USART_Transmit_Newline();						
					}else{
					...
					if(endPtr != extractedString){
						...
						USART_Transmit_Float(value, 2);
						set_desiredRoll(value);	// TODO: added no idea if correct
						USART_Transmit_Newline();						
					}else{
						USART_Transmit_String("ERROR: Parsing failed!\n");						
					}
					...
					if(endPtr != extractedString){
						...
						USART_Transmit_Float(value, 2);
						set_desiredYaw(value);	// TODO: added no idea if correct
						USART_Transmit_Newline();						
					}else{
						USART_Transmit_String("ERROR: Parsing failed!\n");						
					}
